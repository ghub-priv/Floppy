...
MOVIE_FAMILY_CONFIG = {
    "genres": (
        ...
        12.0,
        20,
        "Genre effects against your personal movie-rating baseline.",
    ),
    "decades": (
        "Decades",
        20.0,
        20,
        "Release-decade effects against your personal baseline.",
    ),
    "directors": (
        "Directors",
        6.0,
        4,
        "Strict Director-role credits only; other Directing-department crew are excluded.",
    ),
    "lead_cast": (
        "Lead cast",
        8.0,
        5,
        "Top five cast credits per film, with small samples shrunk towards baseline.",
    ),
    "studios": (
        "Studios",
        15.0,
        10,
        "Production-company effects with a stronger evidence threshold.",
    ),
    "collections": (
        "Collections / franchises",
        8.0,
        4,
        "Provider collection/franchise effects where metadata is available.",
    ),
}
...
TV_FAMILY_CONFIG = {
    "genres": (
        ...
        8.0,
        10,
        "Show-normalised genre effects. Each TV show contributes one observation regardless of episode count.",
    ),
    "decades": (
        "Decades",
        10.0,
        10,
        "Show-normalised release-decade effects against your TV-show baseline.",
    ),
    "lead_cast": (
        "Lead cast",
        6.0,
        3,
        "Top five show-level cast credits where metadata is available.",
    ),
    "studios": (
        "Studios",
        8.0,
        5,
        "Show-level production-company effects with low-sample shrinkage.",
    ),
}
...
def _correlation_label(value):
    ...
    if magnitude < VERY_WEAK_CORRELATION_MAX:
        return "Very weak"
    if magnitude < WEAK_CORRELATION_MAX:
        return "Weak"
    if magnitude < MODERATE_CORRELATION_MAX:
        return "Moderate"
    if magnitude < STRONG_CORRELATION_MAX:
        return "Strong"
    return "Very strong"
    ...
    return [
        {
            ...
            "pct": round(
                (distribution[score] / len(scores) * 100.0) if scores else 0.0, 3
            ),
...
def _load_movies(user):
    ...
    for row in rows:
        ...
        if current is None or (
            stamp is not None and (current_stamp is None or stamp >= current_stamp)
        ):
...
def _load_tv_shows(user):
    ...
    for tv_id in rated_tv_ids:
        ...
        adjusted_rating = (
            confidence * raw_rating + (1.0 - confidence) * global_episode_mean
        )
        ...
        shows.append(
            {
                ...
                "coverage_pct": (rated_count / total_episodes * 100.0)
                if total_episodes
                else 0.0,
...
def _summarise_family(family, config, buckets, baseline, coverage_count, title_count):
    ...
    return {
        ...
        "coverage_pct": round(
            (coverage_count / title_count * 100.0) if title_count else 0.0, 2
        ),
        "distinct_values": len(rows),
        "eligible_rows": sorted(eligible, key=lambda r: (r["label"], r["samples"])),
        "positive": sorted(
            positive, key=lambda r: (r["shrunk_delta"], r["samples"]), reverse=True
        )[:20],
        "negative": sorted(negative, key=lambda r: (r["shrunk_delta"], -r["samples"]))[
            :20
        ],
    }
...
def _compute_combined_profile(user):
    ...
    balanced = (
        mean([movie_mean, tv_mean])
        if movies["unique_rated_titles"] and tv["unique_rated_titles"]
        else (movie_mean or tv_mean or 0.0)
    )
    cross_media_cards = _combine_basic_family_cards(movies, tv)
    shared = sum(card["shared_values"] for card in cross_media_cards)
    consistent = sum(
        len(card["positive"]) + len(card["negative"]) for card in cross_media_cards
    )
    return {
        ...
        "unique_rated_titles": movies["unique_rated_titles"]
        + tv["unique_rated_titles"],
...
def compute_rating_intelligence_profile(user, media_kind="movies"):
    ...
    if scores:
        baseline_payload = {
            ...
            "high_rating_7_plus_pct": round(
                sum(s >= HIGH_RATING_THRESHOLD for s in scores) / len(scores) * 100.0, 3
            ),
            "elite_rating_8_plus_pct": round(
                sum(s >= ELITE_RATING_THRESHOLD for s in scores) / len(scores) * 100.0,
                3,
            ),
        }
    else:
    ...
    profile = {
        ...
        "world_alignment": {
            ...
            "mean_residual_points": round(mean(world_residuals), 4)
            if world_residuals
            else None,
            "median_residual_points": round(median(world_residuals), 4)
            if world_residuals
            else None,
        },
        "families": families,
    ...
    if is_tv:
        ...
        profile["tv_summary"] = {
            ...
            "episode_median": round(median(episode_scores), 4)
            if episode_scores
            else None,
            "episode_stddev": round(pstdev(episode_scores), 4)
            if len(episode_scores) > 1
            else 0.0,
            "episode_7_plus_pct": round(
                sum(s >= HIGH_RATING_THRESHOLD for s in episode_scores)
                / len(episode_scores)
                * 100.0,
                3,
            )
            ...
            "episode_8_plus_pct": round(
                sum(s >= ELITE_RATING_THRESHOLD for s in episode_scores)
                / len(episode_scores)
                * 100.0,
                3,
            )
            ...
            "top_shows": sorted(
                show_rankings,
                key=lambda r: (r["adjusted_rating"], r["rated_episodes"], r["rating"]),
                reverse=True,
            )[:10],
            "bottom_shows": sorted(
                show_rankings,
